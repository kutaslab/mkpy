...
def merge(
    ...
    def _check_typesafe(obj1, obj2, key, path):
        if not key in obj1:
            return
        elif (flags & MERGE_TYPESAFE == MERGE_TYPESAFE) and (
            type(obj1[key]) != type(obj2[key])
        ):
            raise TypeError(
                "Cannot merge objects of type {0} and {1} at {2}"
            ...
            if not v in dst:
                ...
                if not isinstance(src[v], (MutableMapping, MutableSequence)):
                    ...
                    merge(
                        ...
                        afilter=afilter,
                        flags=flags,
                        _path=separator.join([_path, str(v)]),
                        separator=separator,
                    )
            ...
            if dst[dsti] == None:
                ...
                if not isinstance(src[i], (MutableMapping, MutableSequence)):
                    ...
                    merge(
                        ...
                        afilter=afilter,
                        flags=flags,
                        _path=separator.join([_path, str(i)]),
                        separator=separator,
                    )
