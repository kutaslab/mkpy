    ...
    if path_len == len(ss_glob):
        ...
        if PY3:
            return all(
                map(
                    fnmatch.fnmatch,
                    list(map(str, paths_only(path))),
                    list(map(str, ss_glob)),
                )
            )
    ...
    for elem in path:
        ...
        if issubclass(obj.__class__, (MutableMapping)):
            ...
            if not str(elem_value).isdigit():
                raise TypeError(
                    "Can only create integer indexes in lists, "
                    "not {}, in {}".format(type(obj), traversed)
                )
            ...
            raise TypeError(
                "Unable to path into elements of type {} "
    ...
    for pair in path:
        ...
        if view:
            if isinstance(tail, MutableMapping):
                if (
                    issubclass(pair[1], (MutableSequence, MutableMapping))
                    and index != path_count
                ):
                    tail[key] = pair[1]()
                else:
                    tail[key] = target
